...
commonly used in reductions in which several copies of the same variable are created and must be kept consistent.
...
gadget. Therefore \(EQ(a,b)\) translates the ordering restrictions imposed by \(\operatorname{for}_{23}\) into an
...
\]
be an instance in which each clause contains exactly three distinct positive variables.

We first record a local property of \(\operatorname{for}_{23}\).

\medskip
\noindent\textbf{Local rule.}
Let
\[
u_1\prec u_2\prec\cdots\prec u_\ell\prec t
\]
...
Given two existing vertices \(a\) and \(b\), we construct a gadget \(EQ(a,b)\) by adding four private vertices
\[
p,q,r,s.
\]
Each of \(a\) and \(b\) is adjacent to all four internal vertices, and we also add the edges
\[
ps
\qquad\text{and}\qquad
qr.
\]
No other edges are added inside the gadget; in particular, \(a\) and \(b\) are not adjacent.

We claim that every solution with two parts that avoids \(\operatorname{for}_{23}\) satisfies
\[
c(a)=c(b).
\]
...
We order the clauses arbitrarily. After all occurrence blocks, we place all variable vertices \(v_x\), also in
arbitrary order.

We verify the local rule for every vertex.

\begin{itemize}
\item The vertices \(p_o\) and \(q_o\) have no earlier neighbors.

\item The first majority occurrence \(o_1\) has as earlier neighbors only \(p_{o_1}\) and \(q_{o_1}\), both of
...
\item The minority occurrence \(o_3\) has as earlier neighbors the two majority occurrence vertices and
