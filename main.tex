...
C \in \binom{\{1,2,3\}}{2}.
\]
\eric{Cambiar a $C \subseteq \{1,2,3\}$ y hablar de los casos $C = \{1,2,3\}$ y $|C| = 1$.}
...
An (uncolored)\eric{Considerar llamarlo siempre uncolored patterns} \emph{pattern} on three ordered vertices \(r\prec s\prec t\) is a triple
\[
P=(A,N,C),
\]
where
...
\end{definition}
\german{pongo 1<2<3 ?}\eric{Creo que si}
\german{no entendi si habia q cambiar esta def al final pq no te gustaba en C}
\german{tengo q escribir que familia de patrones es una colecion de patrones? es medio obvio}
...
\german{tanto aca como arriba repito estas frases, se podria factor comunizar?}
...
\eric{Usar solo la def de abajo, y decir que partition number es cuando el C está vacío}

\begin{definition}[\(\mathcal C_P\)-chromatic number]
Let
\[
P=(A,N,\varnothing)
\]
be a pattern, and let \(\mathcal C_P\) be the graph class associated with \(P\).

A \emph{\(\mathcal C_P\)-coloring} of a graph \(G\) is a partition
\[
...
The \emph{\(\mathcal C_P\)-chromatic number} of \(G\), denoted by
\[
\chi(G:\mathcal C_P),
...
is the minimum number of parts in a \(\mathcal C_P\)-coloring of \(G\).
\end{definition}



The parameter \(\chi(G:\mathcal C_P)\) is the partition parameter associated with the uncolored pattern \(P\),
whereas \(\minClases_{\mathcal P}(G)\) is defined through a single global order and a partition avoiding a family
of colored patterns.

\[
\minClases_P(G):=\chi(G:\mathcal C_P).
\]
\german{en algun lado quiero poner esto por si no quedo claro}




\german{en teoria esto de abajo es Cp chromatic}
\eric{borrar}
...
\german{subdividir tal vez esta seccion?}
...
\]
\end{lemma}
\german{esta demo quedo muy alargada, quisiera mas cosas en el mismo renglon}
\begin{proof}
First suppose that
\[
\minClases_P(G)=1.
...
In other words, when \(C=\varnothing\), the parameter \(\minClases_P\) is the partition version of the original
...
\german{estos dos estan duplicados, cual te gusta mas sensei?}\eric{Nop son diferentes}
...
P=(A,N,C)
\]
be a colored pattern, let
\[
P'=(A,N,\varnothing)
\]
be the corresponding uncolored pattern, and let \(\mathcal C_{P'}\) be its associated graph class.
Then, for every graph \(G\),
\[
\minClases_P(G)\ge \chi(G:\mathcal C_{P'}).
\]
\end{theorem}


\german{si uso de lo Cp chromatic, tengo q cambiar esto}\eric{Va a ser $\chi_P(G)$}

\begin{theorem}\label{thm:colored-lower-bound-by-class-partition}
Let \(P=(A,N,C)\) be a colored pattern, and consider the pattern $P' = (A,N,\varnothing)$.
...
as desired.
\end{proof}
\german{mas alla de cual de las dos use de arriba, podria tal vez usar que
P' C P para decir esto de arriba?}\eric{Si!}
...
\german{ABAJO  IMP originalmente teniamos que de la igualdad de parametros se deducia esto... entiendo que esta es la forma correcta}
\german{o sea del teorema que habla de dada particion, se deduce el de param generales}\eric{taper}
\begin{theorem}\label{thm:13-fixed-partition-characterization}
Let
\[
P=(A,N,\{1,3\})
\]
...
P'=(A,N,\varnothing)
\]
be the corresponding pattern.

Let \(G\) be a graph and let
\[
\mathcal Q=\{V_1,\dots,V_k\}
\]
be a fixed partition of \(V(G)\). Then, there exists an order \(\prec\) of \(V(G)\) such that
\[
(\prec,\mathcal Q)
\]
avoids \(P\) if and only if \eric{$\mathcal{Q}$ is a $P$-coloring o $\mathcal{C}_P$-coloring}
\[
G[V_i]\in\mathcal C_{P'}
\qquad
\text{for every }i=1,\dots,k.
...
\begin{proof}
Suppose that there exists an order \(\prec\) such that
\[
(\prec,\mathcal Q)
\]
avoids \(P_{13}\). Fix \(i\in\{1,\dots,k\}\), and restrict \(\prec\) to \(V_i\).
\german{lo llamo P13 o P?}\eric{Como quieras, quiza $P13$ es mas consistente? no se}
Let
\[
v_1\prec v_2\prec v_3
\]
be an ordered triple of vertices in \(V_i\). Since \(v_1\) and \(v_3\) lie in the same part of \(\mathcal Q\),
\colorcondition{} does not hold for \(P_{13}\). Therefore, the triple must avoid \(P_{13}\) by
\edgecondition{} or \nonedgecondition{}. Equivalently, it avoids the pattern \(P\).
Hence \[
G[V_i]\in\mathcal C_P.
\]
...
\(\prec_i\) of \(V_i\) that avoids \(P\) in \(G[V_i]\).
...
v_1\prec v_2\prec v_3
\]
be an ordered triple. If \(v_1\) and \(v_3\) lie in different parts of \(\mathcal Q\), then the triple avoids
\(P_{13}\) by \colorcondition.

Suppose instead that \(v_1\) and \(v_3\) lie in the same part \(V_i\). Since the order is defined by blocks and
\[
v_1\prec v_2\prec v_3,
\]
we also have \(v_2\in V_i\). Thus the entire triple lies in \(G[V_i]\), where the order \(\prec_i\) avoids \(P\).
Therefore, the triple avoids \(P_{13}\) by \edgecondition{} or \nonedgecondition{}.

Hence \((\prec,\mathcal Q)\) avoids \(P_{13}\).
\end{proof}
...
\begin{corollary}\label{cor:13-colored-uncolored-equality}
Let
\(
P=(A,N,\{1,3\})
\)
...
\begin{proof}
By Theorem~\ref{thm:colored-lower-bound-by-class-partition},
\[
...
k:=\minClases_{P'}(G).
\]
By definition, there exists a partition
\[
\mathcal Q=\{V_1,\dots,V_k\}
\]
of \(V(G)\) such that
...
By Theorem~\ref{thm:13-fixed-partition-characterization}, applied to this fixed partition \(\mathcal Q\), there exists
a total order \(\prec\) of \(V(G)\) such that
\[
(\prec,\mathcal Q)
\]
avoids \(P\). Therefore, \(\mathcal Q\) is the partition of a valid solution for \(P\) with \(k\) parts. Hence
\[
...
\begin{remark}
For the classes associated with patterns on three vertices considered in this work\eric{En realidad sirve para todos los patrones en 3 vertices}, membership in
\(\mathcal C_P\) can be decided in polynomial time. Hence Theorem~\ref{thm:13-given-partition-polynomial}
...
\subsection{The given-order variant with two parts}
\label{subsec:given-order-two-parts}
...
\(v_i\) and \(v_j\) to lie in different parts of the solution.\eric{Hacer analogía con el $G_<$ de thinness y proper thinness.}
...
\begin{proof}
  \german{del teorema G,< admite una solucion con a lo sumo dos partes sii R es bipartito.}
Let
\[
R=R_{P,D}(G,\prec)
\]
...
\eric{Podemos poner un lema general que diga que un grafo dado un orden admite una particion sii el $R$ admite un coloreo}
...
\]\eric{Antes lo llamamos diferente a esto.}
...
\german{no se si definir todos juntos tipo base uncolored pattenrs
o en cada seccion defino}
...
\german{tengo que tmb citar rtdo general de 13 y explicar pq eso es igual a techo X/2}
...
\subsection{\texorpdfstring{\(\bipparam{23}\)}{bip23}}
\label{subsec:bip23}
\german{esta seccion o podemos borrarla o decir que es simetrica a 13}
...
\eric{Separar en teoremas}
\begin{theorem}\label{thm:bip12-bip12-23-bip12-13}
Let \(G\) be a graph with \(\chi(G)\ge 2\). Then
\[
\bipparam{12}(G)
=
...
\bipparam{12}(G)=\chi(G)-1.
\]
Moreover, since every solution avoiding
\german{citar}
\[
...
\]
also avoids \(\bipparam{12}\), monotonicity under inclusion of pattern families gives
\[
\bipparam{12\text{-}23}(G)
\ge
\bipparam{12}(G)
...
It remains to prove the corresponding upper bounds.

\medskip
\noindent\textbf{The parameter \(\bipparam{12\text{-}23}\).}
...
\(\mathcal B_{12\text{-}23}\), and consequently
\[
\bipparam{12\text{-}23}(G)
\le
k-1
...
\medskip
\noindent\textbf{The parameter \(\bipparam{12\text{-}13}\).}
...
\german{una observcion arriba de todo q me deje tomar directamente una tenra ordenada que no tenga edge condition asi
no repito cada vez esta oracion del otherwise}
...
Hence \(S'\) avoids \(\mathcal B_{12\text{-}13}\), and consequently
\[
\bipparam{12\text{-}13}(G)
\le
k-1
...
Combining all the equalities, we conclude that
\[
\bipparam{12}(G)
=
\bipparam{12\text{-}23}(G)
...
\german{falta????}\eric{Nop está erriba}


\subsection{\texorpdfstring{\(\bipparam{13\text{-}23}\)}{bip13-23}}
\label{subsec:bip13-23}
\german{lo mismo, o borrarla o decir que es simetrica a 12 13}
