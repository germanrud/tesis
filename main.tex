...
Let \(r\prec s\prec t\) be three ordered positions. The colored pattern \(\forrs\) is
\[
\forrs=(A,N,C),
\]
where
...
\textbf{Input:} A graph \(G=(V,E)\), a total order \(\prec\) of \(V\), and an integer \(k\).
...
The \(\forrs\)-Avoidance problem with a given partition can be solved in polynomial time.
\end{theorem}
...
If the algorithm deletes the vertices in the order
\[
d_1,d_2,\dots,d_n,
\]
then the corresponding order is
\[
...
d_1,d_2,\dots,d_n
\]
be the deletion order, and define
\[
d_n\prec d_{n-1}\prec \cdots \prec d_1.
\]
...
We reduce from \textsc{NAE-3SAT}. Let \(\Phi\) be an instance of \textsc{NAE-3SAT}.
...
esta es parecida a la de min clases de arriba de todo, no se si unificarlas, ERIC help me
\end{definition}
...
Let \(P=(A,N,C)\) be a colored pattern, and let \(\mathcal C_P\) be the graph class obtained by forgetting the color constraint of \(P\).
...
\]
Then, for every fixed \(k\ge 3\), the \(\mathcal F_D\)-Avoidance problem with a given order is NP-hard.
\end{lemma}
