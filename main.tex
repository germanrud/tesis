...
The solution \(S\) \emph{avoids} \(P\) in \(G\) if every ordered triple avoids \(P\) \german{in G (?}.
...
\german{pongo A sub eso A sub bip aca?}
\begin{itemize}
...
(y,x,v)
\]
avoid \(P\) in \(H\). \german{importante agregar in H no? tmb lo cambie en la def de arriba de todo}.
\german{hace falta poner las dos triplas? con poner (x,y,z) alcanza no? pq intercambian el papel}
\end{definition}
...
In words, once the last vertex of the ordered triple is fixed, the order of the first two vertices does not matter.
\end{definition}
...
list \(L\) from right to left. We show that \((\prec,\mathcal Q)\) avoids \(\mathcal P\) \german{seguro agregar que si es obvio en que grafo, se omite el in G no?}.
...
\begin{lemma}\label{lem:swap-first-two}
...
swapping the first two vertices. \german{dudo de si esta demo tiene q ser mas formal}
\end{proof}
...
V(X)=\{x_1,\dots,x_n\}.
\]
\german{queda claro que estan ordenados?}
...
different parts. Hence the color condition \(C=\{1,3\}\) of \(\operatorname{for}_{13}\) fails. \german{o prferis asi lo del C}
