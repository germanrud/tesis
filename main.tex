...
We say that the ordered triple \((v_1, v_2, v_3)\) \emph{avoids} \(P\) if at least one of the following conditions holds:
\begin{enumerate}
\item There exists \(\{a,b\}\in A\) such that \(\{v_a, v_b\}\notin E(G)\).
\item There exists \(\{a,b\}\in N\) such that \(\{v_a, v_b\}\in E(G)\).
\item If \(C=\{a,b\}\), then \(v_a\) and \(v_b\) lie in different parts of \(\mathcal Q\).
\end{enumerate}

The solution \(S\) \emph{avoids} \(P\) if every ordered triple avoids \(P\).
...
\[
\{a, b\}\in\binom{\{r,s,t\}}{2},
\]
we define the colored pattern
\[
\operatorname{for}_{ab}=(A,N,\{a, b\}),
\]
where
\[
A=\bigl\{\{r,t\},\{s,t\}\bigr\},
...
\eric{Poner estas tres como subfigures de la misma figure por ej.}
...
When \(\mathcal P=\{P\}\) consists of a single colored pattern, we simply write \(P\)-Avoidance \eric{usar textsc} instead of
\(\{P\}\)-Avoidance.
