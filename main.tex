...
\eric{mepa que esto que viene es una observation o un lemma OK}
...
Una solución \(S=(\prec,\mathcal Q)\) para $G$ se dice \emph{óptima (para \(\mathcal P\))} si \(S\) evita \(\mathcal P\) y
\[
...
Let \(r\prec s\prec t\) be three ordered vertices. For each
\[
ab\in\{rs,rt,st\},
\]
we define the colored pattern
\[
\operatorname{for}_{ab}=(A,N,C_{ab}),
\]
where
\[
A=\bigl\{\{r,t\},\{s,t\}\bigr\},
...
C_{st}:=\{s,t\}.
\]
Thus, all three patterns have the same required edges \(rt\) and \(st\), and differ only in the pair of vertices
required to lie in the same part of the partition.
...
\begin{definition}[Forest colored patterns]
Let \(r\prec s\prec t\) be three ordered vertices. We define the three forest colored patterns
\[
\forrs=(A,N,\{r,s\}),\qquad
\forrt=(A,N,\{r,t\}),\qquad
\forst=(A,N,\{s,t\}),
...
N=\varnothing.
\]
Thus, the three patterns have the same required edges \(rt\) and \(st\), and differ only in the pair of vertices that must lie in the same part of the partition.
\end{definition}
...
Let \(r\prec s\prec t\) be three ordered vertices\eric{Mepa que mejor usemos vertices en todos lados en vez de positions, porque no estoy seguro de qué sea una position DONE}. The colored pattern \(\forrs\) is
\[
\forrs=(A,N,C),
\]
where
...
Decidir cual version preferimos, estas no las borre, agregue arriba la version comprimida
...
\textbf{Input:} A graph \(G=(V(G),E)\) and an integer \(k\).
...
\begin{problem}[\textsc{\(\mathcal P\)-Avoidance with a given order}]
\textbf{Input:} A graph \(G=(V(G),E)\) \eric{Usemos siempre o $V(G)$ o $V$ oK}, a total order \(\prec\) of \(V\), and an integer \(k\).
...
\textbf{Input:} A graph \(G=(V(G),E)\) and a partition
\[
\mathcal Q=\{V_1,\dots,V_k\}
\]
of \(V\).
...
\mathcal Q=\{V_1,\dots,V_k\}
\]
be a fixed partition of \(V(G)\), and let \(H=G[A]\) be an induced subgraph of \(G\), with
\(A\subseteq V(G)\). Let \(\mathcal P\) be a family of colored patterns.

A vertex \(v\in V(H)\) is called \emph{\(\mathcal P\)-removable} in \(H\) with respect to \(\mathcal Q\) if no pattern
in \(\mathcal P\) can be realized with \(v\) playing the role of the last vertex \(t\).

More formally, \(v\) is \(\mathcal P\)-removable if, for every pattern
\[
P\in\mathcal P
\]
and every injective map
\[
\varphi:\{r,s,t\}\to V(H)
\]
such that \(\varphi(t)=v\), the map \(\varphi\) avoids \(P\).

Equivalently, placing \(v\) at the end of the current induced subgraph \(H\) does not create any forbidden occurrence
whose last vertex is \(v\). \eric{esta ok esto?}
\end{definition}


\begin{lemma}\label{lem:removable-forrs}
Let \(G\) be a graph, let \(\mathcal Q=\{V_1,\dots,V_k\}\) be a fixed partition of \(V(G)\),
and let \(H\) be an induced subgraph of \(G\).
A vertex \(v\in V(H)\) is \(\mathcal F_{rs}\)-removable in \(H\) if and only if
\[
|N_H(v)\cap V_i|\le 1
...
\begin{lemma}\label{lem:removable-forrt-st}
Let \(G\) be a graph, let \(\mathcal Q=\{V_1,\dots,V_k\}\) be a fixed partition of \(V(G)\),
and let \(H\) be an induced subgraph of \(G\).
For a vertex \(v\in V(H)\), let \(\mathcal Q(v)\) denote the part of \(\mathcal Q\) containing \(v\).
Then \(v\) is \(\mathcal F_{rt\text{-}st}\)-removable in \(H\) if and only if
\[
\deg_H(v)\le 1
\qquad\text{or}\qquad
...
\begin{lemma}\label{lem:removable-forrs-st-rt}
Let \(G\) be a graph, let \(\mathcal Q=\{V_1,\dots,V_k\}\) be a fixed partition of \(V(G)\),
and let \(H\) be an induced subgraph of \(G\).
For a vertex \(v\in V(H)\), let \(\mathcal Q(v)\) denote the part of \(\mathcal Q\) containing \(v\).
Then \(v\) is \(\mathcal F_{rs\text{-}st\text{-}rt}\)-removable in \(H\) if and only if either
\[
\deg_H(v)\le 1,
\]
...
\begin{lemma}[Correctness of the deletion algorithm]\label{lem:generic-deletion-correctness}
Let \(G\) be a graph, let \(\mathcal Q=\{V_1,\dots,V_k\}\) be a fixed partition of \(V(G)\), and let
\(\mathcal P\) be a family of colored patterns.

Assume that the following property holds for \(\mathcal P\): for every induced subgraph \(H\) of \(G\), if there exists
an order \(\prec\) of \(V(H)\) such that \((\prec,\mathcal Q)\) avoids \(\mathcal P\) in \(H\), then \(H\) contains a
\(\mathcal P\)-removable vertex.

Then Algorithm~\ref{alg:generic-given-partition} returns \textbf{YES} if and only if there exists an order \(\prec\)
of \(V(G)\) such that \((\prec,\mathcal Q)\) avoids \(\mathcal P\).
\end{lemma}

\begin{proof}
First suppose that the algorithm returns \textbf{YES}. Let \(L\) be the deletion list produced by the algorithm, and let
\(\prec\) be the order obtained by reading \(L\) from right to left.

We show that \((\prec,\mathcal Q)\) avoids \(\mathcal P\). Consider any ordered occurrence whose last vertex is \(v\).
When \(v\) was deleted by the algorithm, all vertices that appear before \(v\) in the final order were still active.
Since \(v\) was \(\mathcal P\)-removable in the current induced subgraph, this occurrence avoids every pattern in
\(\mathcal P\). Therefore every ordered occurrence avoids \(\mathcal P\), and hence \((\prec,\mathcal Q)\) avoids
\(\mathcal P\).

Conversely, suppose that there exists an order \(\prec\) of \(V(G)\) such that \((\prec,\mathcal Q)\) avoids
\(\mathcal P\). We show that the algorithm cannot get stuck.

At any step, let \(H=G[A]\) be the current induced subgraph. Since deleting vertices cannot create a forbidden
occurrence, the restriction of the valid order \(\prec\) to \(A\) still avoids \(\mathcal P\) in \(H\).
By the hypothesis of the lemma, \(H\) contains a \(\mathcal P\)-removable vertex. Thus the algorithm always has a
valid choice and cannot return \textbf{NO}.

Therefore the algorithm returns \textbf{YES} exactly when a valid order exists.
\end{proof}
...
\end{definition}
\eric{Uncolored pattern en realidad es un pattern, y lo vamos a definir antes que los colored patterns, así que mejor si no ponemos el vacio al final. Patron = (A, N). DONE}
...
\end{definition}\eric{Esto probablemente haya que ponerlo al principio. OK, pero despues}
...
Let \(P=(A,N,C)\) be a colored pattern, and let $P' = (A,N)$ be a pattern. \eric{Definir un $P' = (A,N)$. OK}
...
\eric{Usar minClases P' en vez de el min este. DONE}
...
CREO que hay que seguir chequeando desde aca
...
% In the preamble, if not already included:
% \usepackage{algorithm}
% \usepackage{algpseudocode}

\begin{definition}[Removable vertex for \(\mathcal F_{rt\text{-}st}\)]
Let \(G=(V,E)\) be a graph and let
\[
\mathcal Q=\{V_1,\dots,V_k\}
\]
be a fixed partition of \(V(G)\). For a vertex \(v\in V(G)\), we denote by
\(\mathcal Q(v)\) the unique part of \(\mathcal Q\) that contains \(v\).

Let \(H\) be an induced subgraph of \(G\). A vertex \(v\in V(H)\) is called
\emph{removable} in \(H\) with respect to \(\mathcal Q\) if at least one of the following holds:
\[
\deg_H(v)\le 1,
\]
or
...
N_H(v)\cap \mathcal Q(v)=\varnothing.
\]
In other words, \(v\) is removable if, in the current induced subgraph, either it has at most one neighbor,
or none of its current neighbors belongs to its own part of the partition.
\end{definition}

\begin{theorem}\label{thm:forrt-st-given-partition-poly}
The \(\mathcal F_{rt\text{-}st}\)-Avoidance problem with a given partition can be solved in polynomial time.
\end{theorem}
...
\mathcal Q=\{V_1,\dots,V_k\}
\]
be the given partition of \(V(G)\). We use the following greedy algorithm.

\begin{algorithm}[ht]
\caption{\textsc{Given-Partition-\(\mathcal F_{rt\text{-}st}\)}}
\label{alg:forrt-st-given-partition}
\begin{algorithmic}[1]
\Require A graph \(G=(V,E)\) and a partition \(\mathcal Q=\{V_1,\dots,V_k\}\) of \(V(G)\).
\Ensure \textbf{YES} and an order \(\prec\) if such an order exists; otherwise \textbf{NO}.
...
\While{\(A\neq\varnothing\)}
    \If{there exists a removable vertex \(v\in A\) in \(G[A]\) with respect to \(\mathcal Q\)}
        \State append \(v\) to \(L\)
        \State \(A\gets A\setminus\{v\}\)
    \Else
        \State \Return \textbf{NO}
...
Suppose that \(H\) is an induced subgraph of \(G\), and that there exists an order \(\prec\) of \(V(H)\)
such that \((\prec,\mathcal Q)\) avoids both \(\forrt\) and \(\forst\) in \(H\).
Then the last vertex of this order is removable in \(H\).

Indeed, let \(v\) be the last vertex of the order. All neighbors of \(v\) in \(H\) lie to the left of \(v\).
If \(\deg_H(v)\le 1\), then \(v\) is removable. Assume then that \(\deg_H(v)\ge 2\).
We claim that \(v\) has no neighbor in its own part.

Suppose, towards a contradiction, that there exists
\[
x\in N_H(v)\cap \mathcal Q(v).
\]
Since \(\deg_H(v)\ge 2\), there is another neighbor \(y\in N_H(v)\), with \(y\neq x\).
Both \(x\) and \(y\) appear before \(v\).

If
\[
x\prec y\prec v,
\]
then the triple \((x,y,v)\) realizes \(\forrt\), because \(x\sim v\), \(y\sim v\), and \(x\) and \(v\)
...
y\prec x\prec v,
\]
then the triple \((y,x,v)\) realizes \(\forst\), because \(y\sim v\), \(x\sim v\), and \(x\) and \(v\)
lie in the same part.

Both cases contradict the assumption that the order avoids \(\forrt\) and \(\forst\). Therefore
\[
N_H(v)\cap \mathcal Q(v)=\varnothing,
\]
and \(v\) is removable.
...
\medskip
\noindent\textbf{If the algorithm returns \textbf{YES}.}
Let \(\prec\) be the order returned by the algorithm; that is, the reverse of the deletion order.
We show that
\[
S=(\prec,\mathcal Q)
\]
avoids both \(\forrt\) and \(\forst\).

Consider an ordered triple
\[
r\prec s\prec t.
\]
When \(t\) was deleted by the algorithm, the vertices \(r\) and \(s\) were still active, because they appear before
\(t\) in the final order. Thus, \(r\) and \(s\) belonged to the current induced subgraph at the moment \(t\) was deleted.

Since \(t\) was removable at that moment, one of the following occurred:

\begin{itemize}
\item \(t\) had at most one neighbor among the active vertices. Then \(t\) cannot be adjacent to both \(r\) and \(s\), so
the triple cannot realize either \(\forrt\) or \(\forst\).

\item \(t\) had no active neighbor in its own part \(\mathcal Q(t)\). Then neither \(r\) nor \(s\), if adjacent to \(t\),
lies in the same part as \(t\). Hence the color condition of both \(\forrt\) and \(\forst\) fails.
\end{itemize}

Therefore no ordered triple realizes \(\forrt\) or \(\forst\), and \(S\) avoids
\(\mathcal F_{rt\text{-}st}\).

\medskip
\noindent\textbf{If there exists a valid order.}
Assume that there exists an order \(\prec\) such that
\[
(\prec,\mathcal Q)
\]
avoids both \(\forrt\) and \(\forst\). We show that the algorithm cannot get stuck.

At any step, let \(H=G[A]\) be the current induced subgraph. The restriction of a valid order to \(A\) still avoids
\(\forrt\) and \(\forst\) in \(H\), since deleting vertices cannot create a forbidden ordered triple.
By the key observation, the last vertex of this restricted order is removable in \(H\). Hence, whenever a valid order
exists for the current subgraph, at least one removable vertex exists.

Thus the algorithm returns \textbf{NO} only if no valid order exists. Therefore, the algorithm returns \textbf{YES}
if and only if there exists an order \(\prec\) such that \((\prec,\mathcal Q)\) avoids both \(\forrt\) and \(\forst\).

Finally, the algorithm runs in polynomial time: at each step we can test removability of all remaining vertices in
polynomial time, and there are at most \(|V(G)|\) deletion steps.
\end{proof}
