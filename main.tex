...
\begin{proof}
By definition, \(\chi_P(G)=1\) if and only if the one-part partition
\(
\{V(G)\}
\)
is a \(P\)-coloring of \(G\). This holds if and only if
\(
G\in\mathcal C_P.
\)
\end{proof}
\german{hace falta la proof?}\eric{Nop}
...
\german{no se si esta remark es muy util}
\begin{remark}
The presence or absence of hyphens is significant. The symbol
\[
P_{123}
\]
denotes the single pattern whose color set is \(C=\{1,2,3\}\), whereas
\[
P_{12\text{-}13\text{-}23}
\]
denotes the family
...
\begin{proof}
By Observation~\ref{obs:chiP-value-one}, it remains only to observe that
a solution of value one is precisely a one-part solution. Hence
\[
\minClases_P(G)=1
\quad\Longleftrightarrow\quad
G\in\mathcal C_P.
...
\eric{Ponele lemiiita pobre}
\begin{observation}[The case \texorpdfstring{\(|C|=1\)}{|C|=1}]
...
\german{hice esto por el comentario de eric que esto era general para las clases q se deciden en tiempo poli}
\german{tal vez es un colorario del 1.88?}
...
\german{dsps de aca hice colorairo, for13 dada particion es poli, pero lo pongo en otra seccion}
...
\)
time.
\german{tema complejidad podemos verlo mejor}
...
\subsection{The base pattern and its colored versions}
\label{subsec:bip-definitions}
\german{te parece bien hacelro asi? y dsps en cada seccion pongo un recordatorio de como seria
siento q no hace falta definir bip12 bip23 y asi pq ya estan defindos entre esto y lo de preliminares}
...
The problem belongs to NP:\eric{Poner esto al principio de todo, que vale para todos los problemas.} a certificate consists of a total order \(\prec\) of \(V(G)\) and a partition
\[
\mathcal Q=\{V_1,\dots,V_q\},
\qquad q\le k,
\]
...
\eric{Eliminar los E y B y F y hacer en todos lados lo que te gusto a vos que me parece lo mejor.}
\german{esto tendria q ir luego de definir los base uncolored patterns}
\begin{definition}[Notation for the induced parameters]
Let
\[
d_1,\dots,d_\ell\in\{12,13,23\}
\]
be distinct indices. We denote the parameters induced by the corresponding colored patterns by
\[
\starparam{d_1\text{-}\cdots\text{-}d_\ell}(G)
:=
\minClases_{
\{
\starparam{d_1},
...
\]
\[
\bipparam{d_1\text{-}\cdots\text{-}d_\ell}(G)
:=
\minClases_{
\{
\bipparam{d_1},
...
and
\[
\forparam{d_1\text{-}\cdots\text{-}d_\ell}(G)
:=
\minClases_{
\{
\forparam{d_1},
...
\bipparam{12}(G)
=
\minClases_{\{\bipparam{12}\}}(G),
\]
\[
\forparam{12\text{-}23}(G)
=
\minClases_{\{\forparam{12},\forparam{23}\}}(G),
\]
and
\[
\starparam{12\text{-}23\text{-}13}(G)
=
\minClases_{
\{
\starparam{12},
...
\begin{remark}
We use the same notation for a colored pattern and for its induced graph parameter. The intended meaning is determined
by whether a graph is given as an argument. For example,
\[
\forparam{12}
\]
denotes the colored pattern, whereas
\[
\forparam{12}(G)
\]
denotes the corresponding parameter evaluated on \(G\). Similarly,
\[
\forparam{12\text{-}23}(G)
\]
denotes the parameter induced by simultaneously forbidding
\(\forparam{12}\) and \(\forparam{23}\).
\end{remark}
...
we define the colored pattern
\[
\operatorname{star}_{ab}:=(A_{\mathrm{star}},N_{\mathrm{star}},\{a,b\}).
\]
...
\draw (v1)--(v2);
\end{tikzpicture}
\caption{\(\operatorname{star}_{12}\)}
\label{fig:star12}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.3\textwidth}
...
\draw (v1)--(v2);
\end{tikzpicture}
\caption{\(\operatorname{star}_{13}\)}
\label{fig:star13}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.3\textwidth}
...
\draw (v1)--(v2);
\end{tikzpicture}
\caption{\(\operatorname{star}_{23}\)}
\label{fig:star23}
\end{subfigure}

\caption{The three basic colored star patterns.}
\label{fig:star-basic-patterns}
\end{figure}
\end{definition}
...
we define the colored pattern
\[
\forparam{ab}:=(A_{\mathrm{for}},N_{\mathrm{for}},\{a,b\}).
\]
...
\qquad
\starparam{12\text{-}23\text{-}13}:=
\{\starparam{12},\starparam{23},\starparam{13}\}.
\]
...
\bipparam{12\text{-}23}
:=
\{\bipparam{12},\bipparam{23}\},
\qquad
\bipparam{12\text{-}13}
:=
\{\bipparam{12},\bipparam{13}\},
\]
\[
\bipparam{13\text{-}23}
:=
\{\bipparam{13},\bipparam{23}\},
\qquad
\bipparam{12\text{-}23\text{-}13}
:=
\{\bipparam{12},\bipparam{23},\bipparam{13}\}.
\]

A solution avoids one of these families if it avoids every colored pattern contained in the family.
For example, a solution avoids \(\bipparam{12\text{-}23}\) if it avoids both
...
\[
\forparam{12\text{-}23}:=
\{\forparam{12},\forparam{23}\},
\qquad
\forparam{12\text{-}13}:=
\{\forparam{12},\forparam{13}\},
\]
\[
\forparam{13\text{-}23}:=
...
\qquad
\forparam{12\text{-}23\text{-}13}:=
\{\forparam{12},\forparam{23},\forparam{13}\}.
\]
A solution avoids one of these families if it avoids every colored pattern in the family.
\end{definition}


\german{estos no se si ponerlos y si ponerlos aca o mas arriba}
...
\]
We keep the given coloring on \(V(X)\)\eric{keep está raro, sería un poco más claro quizá hablando de partition}, assign \(z_i\) to part \(i\) for \(i=1,\dots,k\), and assign
...
\german{este paraemtro ya lo definimos en otro lado, P=parametro}
...
\german{es colorario por mas q se deduzca de algo q esta en preliminares??}
\end{proof}


\eric{El anterior teorema es general para cualquier patron P que sea polinomial de calcular la clase (que creo que son todos).}
\german{lo borre, pero ers for13 with a given a partition}
