...
Dada una terna \((v_1,v_2,v_3)\). (DUDA!!, debo decir que v1<v2<v3 aca no??)\eric{Nop por lo que decís después} de vértices distintos de \(G\), se dice que
...
Dado un grafo \(G\), se define
\[
\estthree(G)\;\coloneq\;\minClases_{\{\mathcalEthree\}}(G),
\]
...
\begin{definition}[Parámetro \(\sigma\)]
  \eric{A eric le gusta: 1. coloneq en vez de :=. 2. $i \in \{1,\dots,k\}$ en vez de $i=1,\dots,k$.}
...
\tau\bigl(G[C_i]\bigr)\le 1
\]
significa que, dentro de la clase \(C_i\), todas las aristas internas son incidentes a un mismo vértice.
...
\[
\estthree(G)=\sigma(G).\qedhere
\]
\eric{Poner qedhere si terminas la demo con una formula}
\end{proof}
...
\quad\Longleftrightarrow\quad
\sigma(G)\le k.
\]\eric{No usar símbolos de lógica para reemplazar texto. Escribir "si y solo si".}
...
\(G[C_i]\) son incidentes a un mismo vértice.
...
El algoritmo es polinomial\eric{Poner especificamente las complejidades temporales de los algoritmos polinomiales. En este caso es $O(n^2)$}, ya que hay
\[
1+|V(G)|+\binom{|V(G)|}{2}
\]
